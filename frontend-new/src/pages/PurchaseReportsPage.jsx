                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div className="flex items-start gap-5">
                        ...
                        <div className="space-y-1">
                            <p className="text-[10px] font-black text-emerald-600 uppercase tracking-[0.3em]">Buy Reports</p>
                            <h1 className="text-4xl font-black text-slate-900 tracking-tight">Buy Records</h1>
                            <p className="text-sm font-bold text-slate-500 pt-1">Review your shop purchases.</p>
                        </div>
                    </div>
            ...
            <div className="glass-card p-0 border-none overflow-hidden" id="printable-report">
                    ...
                    <div className="flex gap-4">
                        <div className="px-8 py-5 bg-emerald-900 text-white rounded-4xl shadow-xl shadow-emerald-900/20 relative overflow-hidden group min-w-[240px]">
                            ...
                            <div className="relative z-10">
                                <p className="text-[9px] font-black uppercase tracking-[0.2em] text-emerald-300 mb-1">Total Buy</p>
                                <h3 className="text-3xl font-black tracking-tighter">₹{totalPurchase.toLocaleString('en-IN')}</h3>
                            </div>
                            <ArrowDownRight size={32} className="absolute bottom-4 right-4 text-white/5" />
                        </div>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-slate-50/50">
                                ...
                                <th className="px-8 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest text-right">Taxable Amount</th>
                            ...
                            {reportData.length > 0 ? (
                                <>
                                        ...
                                        <tr key={row.sno} className="hover:bg-slate-50/50 transition-colors group">
                                            ...
                                            <td className="px-8 py-6">
                                                <div className="flex flex-col">
                                                    <span className="text-sm font-black text-slate-900 tracking-tight uppercase group-hover:text-emerald-600 transition-colors">{row.invNo}</span>
                                    ...
                                    <tr className="bg-emerald-900 text-white">
                                        <td colSpan="7" className="px-8 py-8 text-right text-[10px] font-black uppercase tracking-[0.4em] text-emerald-400">Total</td>
                                        <td className="px-8 py-8 text-right font-black text-2xl tracking-tighter text-white">₹{totalPurchase.toLocaleString('en-IN')}</td>
                                    </tr>
                                </>
                            ) : (
                                <tr>
                                    <td colSpan="8" className="px-8 py-32 text-center">
                                        <div className="flex flex-col items-center justify-center gap-4">
                                            ...
                                            <h4 className="text-lg font-black text-slate-900 tracking-tight">{isLoading ? 'Searching purchases...' : 'No records found'}</h4>
                                            <p className="text-sm font-bold text-slate-500 uppercase tracking-widest">Search to view your purchase records</p>
                                        </div>
                                    </td>
                                </tr>
                            )}
                ...
                <div className="p-10 bg-slate-50/50 flex flex-col md:flex-row items-center justify-between gap-8 border-t border-slate-100">
                    <div className="flex items-center gap-3">
                        ...
                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Purchase Report Summary</span>
                    </div>
                    <div className="flex gap-4">
                        <button
                            onClick={handlePrint}
                            className="px-8 py-4 bg-white border border-slate-200 text-slate-900 rounded-4xl font-black uppercase tracking-widest text-[10px] hover:bg-slate-50 transition-all flex items-center gap-3 shadow-sm"
                        >
                            <Printer size={16} /> Print Report
                        </button>
                        <button
                            onClick={() => setShowInvoiceModal(true)}
                            className="px-10 py-4 bg-emerald-600 text-white rounded-4xl font-black uppercase tracking-widest text-[10px] hover:bg-emerald-700 transition-all shadow-xl shadow-emerald-600/10 active:scale-95 flex items-center gap-3"
                        >
            ...
            {showInvoiceModal && (
                <div className="modal-overlay bg-slate-900/70 backdrop-blur-md p-4" onClick={() => setShowInvoiceModal(false)}>
                    <div className="modal-content max-w-6xl border-none animate-scale-up" onClick={(e) => e.stopPropagation()}>
                        <div className="p-8 border-b flex items-center justify-between bg-slate-900 text-white rounded-t-4xl">
                            <div className="flex items-center gap-6">
                                ...
                                <div>
                                    <h3 className="text-xl font-black tracking-tighter">Purchase Invoice</h3>
                                    <p className="text-[9px] font-black text-slate-400 uppercase tracking-[0.3em] mt-1">Report Details</p>
                                </div>
                            </div>
                            <div className="flex gap-3">
                                <button onClick={handlePrint} className="w-12 h-12 rounded-xl bg-white/10 hover:bg-emerald-600 text-white flex items-center justify-center transition-all border border-white/10">
                                    <Printer size={20} />
                                </button>
                        ...
                        <div className="p-0 overflow-y-auto max-h-[75vh] bg-slate-100 custom-scrollbar">
                            <div className="max-w-[850px] mx-auto my-16 shadow-[0_50px_100px_-20px_rgba(0,0,0,0.2)] bg-white overflow-hidden rounded-[2.5rem] border border-slate-100 p-16">
                                <ReportHeader reportTitle="Buy Report" fromDate={formatDate(fromDate)} toDate={formatDate(toDate)} />
                                <table className="w-full border-collapse mt-12">
                                    <thead>
                                        <tr className="bg-slate-50 border-y-2 border-slate-200">
                                            ...
                                            <th className="text-left py-5 px-4 font-black text-slate-900 text-[10px] uppercase tracking-widest">Invoice No</th>
                                            <th className="text-left py-5 px-4 font-black text-slate-900 text-[10px] uppercase tracking-widest">Item Name</th>
                                    ...
                                    <tbody className="divide-y divide-slate-100">
                                        ...
                                        <tr className="bg-emerald-900 text-white">
                                            <td colSpan="6" className="py-6 px-4 text-right font-black text-[10px] uppercase tracking-[0.4em] text-emerald-400">Total Buy</td>
                                            <td className="py-6 px-4 text-right font-black text-white text-lg tracking-tighter">₹{totalPurchase.toLocaleString('en-IN')}</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
                        ...
                        <div className="p-8 bg-white border-t border-slate-100 flex justify-between items-center rounded-b-[2.5rem]">
                             <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest italic">Buy details</p>
                             <button onClick={() => setShowInvoiceModal(false)} className="px-10 py-4 bg-slate-900 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-emerald-600 transition-all shadow-xl shadow-slate-900/10">Close</button>
                        </div>
                    </div>
                </div>
            )}
